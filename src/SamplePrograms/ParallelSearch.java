...
public class ParallelSearch {
    ...
    public static int search(String itemname) {
        for (int i = 0; i < product.length; i++) {
            if(product[i].equals(itemname)) {
                return i;
            }
        }
        return -1;
    ...
    public static void main(String[] args) {
        ...
        prod = s.nextLine(); 
        int product = search(prod);
        if(product != -1) {
            System.out.println("Price: " + price[product]);
            System.out.println("Category: " + category[product]);
        }else{
            System.out.println("Unable to identify a product with this name.");
        }
    }
}
